...
=item * @stuff = $get->NS(\$buffer,$offset);
...
All except the last item, B<$nsdname>, is provided by
the class loader, B<Net::DNS::ToolKit::RR>. The code in this method knows
how to retrieve B<$nsdname>.
...
The method will insert the $rdlength and $nsdname, then
...
=item * (@COMMON,$NSDNAME) = $parse->NS(@common,$nsdname);
...
For NS RR's, this returns $nsdname terminated with '.'
